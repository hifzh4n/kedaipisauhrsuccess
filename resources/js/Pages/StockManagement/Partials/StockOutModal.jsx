...
export default function StockOutModal({ show, onClose }) {
            ...
            const timer = setTimeout(async () => {
                try {
                    ...
                    if (response.ok) {
                        ...
                        // Transform results to include quantity for display
    ...
    const handleItemSelect = (item) => {
        if ((item.quantity || 0) === 0) {
            toast.warning(
                `Cannot select "${item.item_name}" - Item has 0 quantity in stock`
            );
            return;
        }
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        console.log("Stock Out Modal - Form data being submitted:", data);
        post(route("stocks.out"), {
            onSuccess: () => {
                reset();
            ...
            onError: (errors) => {
                console.error("Stock Out Modal - Validation errors:", errors);
            },
        });
    };
            ...
            <div className="flex flex-col max-h-[90vh]">
                ...
                <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        Remove stock from inventory by selecting an item and
                            ...
                            {(searchResults.length > 0 ||
                                ...
                                <div className="absolute z-10 mt-1 w-full bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-md shadow-lg max-h-60 overflow-y-auto">
                                    {isSearching ? (
                                        ...
                                        <div className="px-4 py-3 text-sm text-amber-600 dark:text-amber-400">
                                            {noStockMessage}
                                        </div>
                                    ) : searchResults.length === 0 ? (
                                        <div className="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                                            ...
                                            <button
                                                ...
                                                className={`w-full px-4 py-3 text-left focus:outline-none ${
                                                    (item.quantity || 0) > 0
                                                        ...
                                                        : "cursor-not-allowed bg-gray-50 dark:bg-gray-800/50 opacity-60"
                                                }`}
                                            >
                            ...
                            {data.reason === "damage" && (
                                <div>
                                    ...
                                    <TextInput
                                        ...
                                        required={data.reason === "damage"}
                                    />
                                    <InputError
                                        message={errors.damage_reason}
                                        className="mt-2"
                                    />
                                </div>
                            )}
